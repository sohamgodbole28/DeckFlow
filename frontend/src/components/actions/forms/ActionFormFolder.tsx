...
export default function ActionFormFolder({ action, onChange }: { action: any, onChange: (config: any) => void }) {
  ...
  const handleBrowse = async () => {
    const path = prompt("Enter the absolute path to the folder:");
    if (path) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        <label className="text-xs font-semibold text-textMuted uppercase tracking-wider">Folder Path</label>
